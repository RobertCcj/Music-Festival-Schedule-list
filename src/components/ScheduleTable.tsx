...
import { Performance, Stage } from '../types/schedule';
...
interface ScheduleTableProps {
  ...
  onPerformanceToggle: (id: string) => void;
  performances: Performance[];
  date: string;
}

const calculateTimeSpan = (startTime: string, endTime: string): number => {
  const startIndex = timeSlots.indexOf(startTime);
  const endIndex = timeSlots.indexOf(endTime);
  return Math.max(1, endIndex - startIndex + 1);
};

function ScheduleTable({ stages, selectedPerformances, onPerformanceToggle, performances, date }: ScheduleTableProps) {
  const getPerformanceStyle = (performance: Performance, stage: Stage) => {
  ...
  const formatPerformanceName = (name: string, country?: string): string => {
    return country ? `${name}\n[${country}]` : name;
  };
